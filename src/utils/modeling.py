"""
File used to determine the models used during training. Mainly based on [GitHub repository](https://github.com/intersun/PKD-for-BERT-Model-Compression) for [Patient Knowledge Distillation for BERT Model Compression](https://arxiv.org/abs/1908.09355).
"""
...
from BERT.pytorch_pretrained_bert.modeling import BertPreTrainedModel, BertModel, BertEmbeddings, BertEncoder, BertModel_ER
...
class BertForSequenceClassificationEncoder(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, rand_shuffle = None):
        if self.output_all_encoded_layers:
            full_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True, rand_shuffle = rand_shuffle)
            return [full_output[i][:, 0] for i in range(len(full_output))], pooled_output
        else:
            _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, rand_shuffle = rand_shuffle)
            return None, pooled_output

class BertForSequenceClassificationEncoder_ER(BertPreTrainedModel):
    def __init__(self, config, output_all_encoded_layers=False, num_hidden_layers=None, fix_pooler=False):
        super(BertForSequenceClassificationEncoder_ER, self).__init__(config)
        if num_hidden_layers is not None:
            logger.info('num hidden layer is set as %d' % num_hidden_layers)
            config.num_hidden_layers = num_hidden_layers
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, rand_shuffle = None):
        if self.output_all_encoded_layers:
            ...
            _, pooled_output = self.bert.forward1(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, rand_shuffle = rand_shuffle)
            return None, pooled_output  
