...
from utils.data_processing import init_model, get_task_dataloader, init_model_SPS_2, init_model_SPS_1, init_model_EC_3, init_model_EC_3_Adj, init_model_EC, init_model_EC_Adj, init_model_New_3
...
from utils.utils import load_model, count_parameters, eval_model_dataloader_nli, eval_model_dataloader, compute_metrics, load_model_finetune, eval_model_dataloader_nli_New
...
if args.kd_model.lower() in ['kd', 'kd.cls', 'kd.u', 'kd.i']:
    ...
    # Specify Model Type
    if args.model_type == 'New':
        if args.student_hidden_layers == 6:
            student_encoder, student_classifier = init_model_New_3(task_name, output_all_layers, args.student_hidden_layers, student_config, shuffle = False)
        elif args.student_hidden_layers == 600:
            student_encoder, student_classifier = init_model_New(task_name, output_all_layers, args.student_hidden_layers, student_config, shuffle = False)    
...
if args.do_train:
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            perm = np.random.permutation(args.student_hidden_layers)
            perm = [0,1,2,3,4,5]
            shuf = np.random.randint(0,2,args.student_hidden_layers)
            shuf = (shuf==1)
            
            if global_step % n == 0 :
                shuf = [False, False, False, False, False, False]
            ...
            full_output, pooled_output = student_encoder(input_ids, segment_ids, input_mask, permutation = perm, rand_shuffle = shuf)    
...
#                         param_group['lr'] = lr_this_step
        ...
        #Save a trained model and the associated configuration
            if (global_step % 2 == 1) & (epoch > -1): 
                if 'race' in task_name:
                    result = eval_model_dataloader_nli(student_encoder, student_classifier, eval_dataloader, device, False)
                else:
                    test_res = eval_model_dataloader_nli_New(args.task_name.lower(), eval_label_ids, student_encoder, student_classifier, eval_dataloader, args.kd_model, num_labels, device, args.weights, args.fc_layer_idx, output_mode, permutation = perm, rand_shuffle = [False, False, False, False, False, False])
                    #test_res = eval_model_dataloader_nli_New(args.task_name.lower(), eval_label_ids, student_encoder, student_classifier, eval_dataloader, args.kd_model, num_labels, device, args.weights, args.fc_layer_idx, output_mode, permutation = [0,1,2], rand_shuffle = [False, False, False])
                    
                #print("")    
                #print("Current Loss: "+str(test_res['eval_loss'])+", and current permutation: ["+str(perm[0])+", "+str(perm[1])+", "+str(perm[2])+"], and shuffle is ["+str(shuf[0])+", "+str(shuf[1])+", "+str(shuf[2])+"]")    
                ...
                if task_name == 'cola':
                    if test_res['mcc'] > eval_best_acc:
                        ...
                        print("Current Shuffle state is "+str(shuf))    
